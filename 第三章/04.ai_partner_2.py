...
def save_background_settings():
    ...
    if not os.path.exists("resources"):
        os.mkdir("resources")
    with open("resources/background_settings.json", "w", encoding="utf-8") as f:
...
def load_background_settings():
    ...
    try:
        if os.path.exists("resources/background_settings.json"):
            with open("resources/background_settings.json", "r", encoding="utf-8") as f:
                return json.load(f)
    except Exception:
        pass
...
st.title("AI智能伴侣")
# logo
st.logo("resources/aixin logo.png")
...
with st.sidebar:#---with, 创建一个上下文管理器，可以在with内部执行代码，并在with外部执行代码
    ...
    if uploaded_file is not None:
        ...
        #保存图片到resources目录
        with open(f"resources/custom_background.png", "wb") as f:
            f.write(image_bytes)
    ...
    if st.session_state.background_image:
        if st.button("删除背景图", icon="❌", width="stretch"):
            ...
            #删除保存的背景图文件
            if os.path.exists("resources/custom_background.png"):
                os.remove("resources/custom_background.png")
            save_background_settings()
            st.rerun()
